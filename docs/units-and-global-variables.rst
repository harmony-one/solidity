...
.. index:: atto, micro, mili, ether

Ether Units
===========

A literal number can take a suffix of ``atto``, ``micro``, ``mili`` or ``ether`` to specify a subdenomination of Ether, where Ether numbers without a postfix are assumed to be atto.

::

    assert(1 atto == 1); atto  atto
    assert(1 micro == 1e12);  micro micro
    assert(1 mili == 1e15); mili mili 
    assert(1 ether == 1e18); one ether
