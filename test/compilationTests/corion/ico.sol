...
contract ico is safeMath {
    ...
    function closeICO() external {
        /*
            ...
            Ones which are situated in this contract will be sent to the address of the fundation.
        */
        require( msg.sender == owner );
        require( block.number > icoDelay );
        require( ! closed );
    ...
    function buy(address payable beneficiaryAddress, address affilateAddress) public payable returns (bool success) {
        /*
            ...
            If there is not at least 0.2 one balance on the beneficiaryAddress then the amount of the one which was intended for the purchase will be reduced by 0.2 and that will be sent to the address of the beneficiary.
        ...
        */
        ...
        if ( beneficiaryAddress.balance < 0.2 one ) {
            require( beneficiaryAddress.send(0.2 one) );
            _value = safeSub(_value, 0.2 one);
        }
    ...
    function getIcoReward(uint256 value) public view returns (uint256 reward) {
        /*
            ...
            @value The amount of one for the purchase
        ...
        */
        reward = (value * 1e6 * icoExchangeRate / icoExchangeRateM / 1 one) * (ICObonus() + 100) / 100;
